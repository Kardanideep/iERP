      ...
      <div className="mx-auto w-full max-w-5xl">
        ...
        <div className="flex flex-col">
          {productData.whyNeed.challenges?.map((item, i) => (
            <div 
              key={i} 
              className="group relative flex flex-col items-start gap-6 border-t border-white/10 py-12 transition-all duration-500 first:border-t-0 hover:px-8"
            >
              {/* Background Highlight on Hover */}
              <div className="absolute inset-0 -z-10 bg-linear-to-r from-premium-burgundy/10 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
              
              <div className="flex w-full items-center justify-between gap-10">
                <div className="flex items-center gap-10">
                   {/* Large Index Number */}
                   <span className="text-4xl font-black text-white/20 transition-colors duration-500 group-hover:text-premium-cream md:text-6xl">
                     {String(i + 1).padStart(2, '0')}
                   </span>
                   
                   {/* Issue Text */}
                   <h3 className="text-2xl font-bold text-premium-cream transition-all duration-500 group-hover:translate-x-2 md:text-3xl">
                     {item.problem}
                   </h3>
                </div>

                {/* Status Indicator */}
                <div className="hidden shrink-0 items-center gap-3 rounded-full border border-premium-burgundy/50 bg-premium-cream px-5 py-2.5 text-xs font-bold text-premium-burgundy shadow-[0_0_15px_rgba(160,30,40,0.2)] transition-all duration-500 group-hover:bg-premium-burgundy group-hover:text-white group-hover:shadow-[0_0_25px_rgba(160,30,40,0.4)] md:flex">
                  <ShieldAlert size={14} className="text-premium-burgundy transition-colors duration-500 group-hover:text-white" />
                  CRITICAL HURDLE
                </div>
              </div>

              {/* Detail / Solution reveal - Deep Technical Explanation */}
              <div className="max-h-0 overflow-hidden transition-all duration-700 group-hover:max-h-[200px]">
              ...
              <div className="absolute bottom-0 left-0 h-[1px] w-0 bg-gradient-to-r from-premium-burgundy to-premium-brown transition-all duration-700 group-hover:w-full" />
            </div>
          ))}
